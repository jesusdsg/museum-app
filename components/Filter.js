...
function Filter({ selectOptions, works, setWorks, API, initData }) {
  ...
  const queryHandler = (event) => {
    if (event == "" && works.length == 0) {
      setWorks(...[initData]);
    } else if (event != "" && maker.value != undefined) {
  ...
  return (
    <div className="bg-white px-8 py-8 mt-8 rounded-lg shadow-lg">
      ...
      <div className="lg:flex grid gap-4">
        ...
        />
        <input
          className="fix-margin-t border-slate-300 border-2 lg:w-1/2 w-full rounded-lg px-4 py-2 h-10 placeholder:text-gray-400"
          name="search"
          type="text"
